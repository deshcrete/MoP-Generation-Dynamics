"""Where do the mixture model's free generations sit in persona-representation space?

Exp 5 draws the persona reference clusters (real D_i stories + base-model free generations for the
base cluster, all embedded through P_mix) and traces ONE rollout's prefix-embedding path across
them. This script keeps the identical cluster background and PCA, but instead of a trajectory it
scatters the POPULATION of P_mix's own free generations — one point per rollout, each the same
mean-pooled embedding object as a cluster point (embed_traj.sequence_embeddings).

The question it answers is the population version of Exp 5's single-rollout read: does free
generation's mass sit on the base region, spread over the behavioural clusters, or reach the
triggered ones? Exp 2 answers this from sequence log-probs (the hard-assignment histogram); this
answers it from the geometry of the representation, independently.
...
Conventions are inherited from Exp 5 (see control/notes.md and context/results.md §Exp 5):
  ...
  - Free generations are embedded under the FORWARD mask (token_dist.forward_attention_mask), which
    ...
    data.tokenize_stories) and the base cluster are embedded, so all points are the same object.
...
Because PC1+PC2 capture only ~1/3 of the variance, 2D distances are illustrative, not metric. So the
plot ships with a quantitative companion, `nearest_centroid.csv`: each generation is assigned to the
nearest cluster centroid in the FULL normalised hidden space (not the 2D projection), giving an
honest "where is the mass" table that does not depend on the projection.
...
from . import config, data, embed_traj, generate, models, token_dist
...
def main() -> None:
    ...
    # =====================================================================================
    print(f"[gen_clusters] building clusters: {N_CLUSTER} D_i stories/persona + {N_CLUSTER} base free-gens ...")
    ...
    print(f"[gen_clusters] embedding {free.shape[0]} P_mix free generations ...")
    ...
    embed_traj.plot_generation_scatter(
        ...
        "P_mix free generations over the persona clusters (PCA of D_i, mean-pooled)",
    ...
    # whose PC1+PC2 hold only ~1/3 of the variance — see context/results.md §Caveats).
    # =====================================================================================
