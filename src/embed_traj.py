...
def plot_generation_scatter(cluster_proj: np.ndarray, cluster_names: list[str], components: list[str],
    ...
    to the persona clusters", not "how does a single rollout get there". gen_proj is [N, 2], already
    ...
    # cluster legend entries are the centroid labels drawn by _scatter_clusters; only the
    # generation cloud needs a key, since it is the one series with no in-plot label.
